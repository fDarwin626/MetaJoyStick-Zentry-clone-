...
// ─── COMPONENT ─────────────────────────────────────────────────────────────────
// onReady — called once the minimum videos have loaded; App.jsx uses this
//           to dismiss the global loading spinner.
const Hero = ({ onReady }) => {
  ...
  // Fire onReady once the background + portal videos have loaded (2 videos).
  // Using a ref guard so it only fires once ever.
  useEffect(() => {
  ...
  // ── Mini hover: lazy-load upcoming ───────────────────────────────────────
  const handleMiniVideoHover = () => {
    if (!nextPreloaded) setNextPreloaded(true);
  };
      ...
      >
        ...
        {/*
          ── LAYER STACK (bottom → top) ──────────────────────────────────────
            z-[0]  outgoingBg  — old video fading out
            z-[1]  incomingBg  — new video playing underneath
            z-[20] next-video  — portal expanding to fullscreen
            z-[50] mini        — small clickable preview
            z-[55] ripple      — warp burst (pointer-events none)
            z-[40] text / CTA
          ──────────────────────────────────────────────────────────────────── */}
        ...
        >
          <source src={bgAssets.webm} type="video/webm" />
        ...
        <div>
          <div className="mask-clip-path absolute-center absolute z-50 size-64 cursor-pointer rounded-3xl overflow-hidden">
            ...
            >
              <video
                src={nextPreloaded ? miniAssets.mp4 : undefined}
                ref={currentVideoRef}
                loop muted playsInline
                preload={nextPreloaded ? "metadata" : "none"}
          ...
          <video
            ...
            onLoadedData={handleVideoLoad}
          >
            <source src={nextAssets.webm} type="video/webm" />
